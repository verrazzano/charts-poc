...
func NewCmdDiff(vzHelper helpers.VZHelper) *cobra.Command {
	cmd := cmdhelpers.NewCommand(vzHelper, CommandName, helpShort, helpLong)
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		return runCmdDiff(cmd, vzHelper)
	}
...
func runCmdDiff(cmd *cobra.Command, vzHelper helpers.VZHelper) error {
	...
	patchFile, err := fs.GeneratePatchWithSourceDir(chart, version, chartsDir, sourceDir)
	if err != nil {
		return err
	}
